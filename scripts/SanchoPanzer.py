...
last_enemy_radarx = 0.0
last_enemy_radarz = 0.0
aim_locked = False
aim_lock_shots = 0
...
class Controller:
    ...
    def run(self):
        ...
        global last_enemy_radarx
        global last_enemy_radarz
        global aim_locked
        global aim_lock_shots
        ...
        while (True):
            ...
            try:
                ...
                # =========================
                # TURRET DECLINATION CONTROL
                # =========================

                enemy_health = float(othervalues[td['health']])
                enemy_radarx = float(othervalues[td['radarx']])
                enemy_radarz = float(othervalues[td['radarz']])

                enemy_was_hit = False
                new_radar_info = False
                ...
                if enemy_radarx != 0.0 or enemy_radarz != 0.0:
                    if enemy_radarx != last_enemy_radarx or enemy_radarz != last_enemy_radarz:
                        new_radar_info = True
                ...
                if enemy_was_hit:
                    ...
                    if aim_lock_shots >= 2:
                        ...
                        if turretdeclination > 1.0:
                            turretdeclination = -0.6

                elif new_radar_info:

                    shot_distance = math.sqrt(
                        (enemy_radarx - float(myvalues[td['x']])) ** 2 +
                        (enemy_radarz - float(myvalues[td['z']])) ** 2
                    )

                    if shot_distance < targetdistance:
                        turretdeclination += 0.1
                    else:
                        turretdeclination -= 0.1

                    if turretdeclination > 1.0:
                        turretdeclination = 1.0
                ...
                last_enemy_radarx = enemy_radarx
                last_enemy_radarz = enemy_radarz
                ...
                if targetdistance < 500:
                    turretdeclination = np.random.uniform(-0.6, 0.0)
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 2:
        ...
        print("Usage: python SanchoPanzer.py [tank_number]")
